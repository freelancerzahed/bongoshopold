...
class CheckoutController extends Controller
{
    ...
    public function store_shipping_info(Request $request)
    {
        ...
        if(Auth::check() &&  Auth::user()->email_verified_at == NULL){
            ...
            sendSMS($user->phone, 2 , $user->verification_code);
            flash(__('Please submit 6 digits otp'))->success();
        ...
        if (Auth::check()) {
            ...
            $pin_code = PinCode::where('pin_code',$address->postal_code)->first();
            if($pin_code){
                $data['postal_code'] = $address->postal_code;
            }else{
                flash(__('Invalide Pin Code; Please add another pin Code'))->error();
        return back();
            }
            ...
            $pin_code = PinCode::where('pin_code',$request->postal_code)->first();
            if($pin_code){
