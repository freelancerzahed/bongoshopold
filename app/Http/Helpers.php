...
if (! function_exists('sendSMS')) {
    function sendSMS($to, $from, $text)
    {
        ...
        }elseif(OtpConfiguration::where('type', 'sms_alert')->first()->value == 1){

          try{
            $SMSALERT_USER = env('SMSALERT_USER'); //change here
            $SMSALERT_PWD  = env('SMSALERT_PWD'); //change here
            $smsalert      = (new Smsalert())
		->authWithUserIdPwd($SMSALERT_USER,$SMSALERT_PWD);
            $smsalert->setSender($from)
            ->send($to, $text);
