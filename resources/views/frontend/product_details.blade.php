                ...
                <div class="row no-gutters cols-xs-space cols-sm-space cols-md-space">
                    ...
                    <div class="col-lg-6">
                        ...
                        <div class="product-description-wrapper">
                            ...
                            <div class="row no-gutters mt-4">
                                <div class="col-2">
                                    <div class="product-description-label mt-2">{{ translate('Check Pin')}}:</div>
                                </div>
                                <div class="col-10">
                                   @if(Auth::check())
                                   @php
                                   $address= DB::table('addresses')->where('user_id', Auth::user()->id)->first();
                                    @endphp
                                   <div id="pinCode">
                                    <input class='py-1' id="pinCodeValue" 
                                    ...
                                    <button id='pinCodeCheck' class="btn btn-warning .text-white ">Check</button>
                                    </div>
                                    @else
                                    <div id="pinCode">
                                    <input class='py-1' id="pinCodeValue"  type="number" placeholder='Enter pincode'>
                                   
                                    <button id='pinCodeCheck' class="btn btn-warning .text-white ">Check</button>
                                    </div>
                                   @endif
                                </div>
                                <div class="col-10" id='ShowPinDetails'>
...
let pinCode = $('#pinCodeValue').val()
if(pinCode==''){
    alert('Please enter Pin Code')
}else{
    var urlpin = "{{url('get-pin-code/')}}"+'/'+pinCode
$.ajax({  //create an ajax request to display.php
          type: "GET",
          url: urlpin,       
          success: function (data, status) {
         if(status=='success'){
            $('#ShowPinDetails').empty()
          $('#ShowPinDetails').append(`
         <p style="color:green; font-size:18px; margin-top:10px;">Delivery by ${data.delivery_time} dayes <br> in ${data.city}</p>
          ...
          `)
            ...
            error: function (data) {
                $('#ShowPinDetails').empty()
          $('#ShowPinDetails').append(`
         <p style="color:red; font-size:18px; margin-top:10px;">Not Available For Delivery </p>
